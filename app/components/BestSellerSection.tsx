...
export function BestSellerSection({ products }: { products: any }) {
    ...
    function handleQuickBuy(product: any) {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-10 md:mb-14">
                    ...
                    <p className="text-amber-800/70 text-base md:text-lg max-w-7xl mx-auto">
                        Discover our best-selling products, carefully curated to meet your needs.
                    </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 p-4 bg-amber-200 border-amber-600 border-1">
                    {products?.nodes?.map((product: any, index: number) => (
                        <ProductItem key={product.id} product={product} handleQuickBuy={handleQuickBuy} />
                    ))}
                </div>
                <QuickCart selectedProduct={selectedProduct} handleCloseCart={handleCloseCart}/>
            </div>
        </section>
    );
}